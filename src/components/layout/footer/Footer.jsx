        ...
        <footer className="bg-navbar-web min-h-32 lg:min-h-[400px] text-gray-200 py-10 lg:flex flex-col justify-center">
            <div className="grid gap-5 lg:grid-cols-2">
                ...
                <div className="sm:grid grid-cols-2 justify-around items-center">
                    <FooterLista items={lista} />
                    <FooterLista items={listaRecursos} />
                </div>
            </div>
...
const FooterLista = ({ items = [] }) => {
    return (
        <div className="text-center border-t py-5 border-gray-600 sm:border-x sm:border-y-0 sm:px-10 lg:text-lg font-extralight">
            <ul>
                {items.map((item) => (
                    <li key={item} className="hover:text-blue-300">
